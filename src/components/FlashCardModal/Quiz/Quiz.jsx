...
export default function PlayCountdown({
    ...
    const cardImgs = useRef([...cards.map(i => i.frontImage), ...cards.map(i => i.backImage)].filter(i => !!i))
    const cardTexts = useRef([...cards.map(i => i.frontText), ...cards.map(i => i.frontText)].filter(i => !!i))

    const frontAnswersText = useRef([...cards.map(i => i.frontText).filter(i => !!i)])
    const frontAnswersImage = useRef([...cards.map(i => i.frontImage).filter(i => !!i)])
    const backAnswersText = useRef([...cards.map(i => i.backText).filter(i => !!i)])
    const backAnswersImage = useRef([...cards.map(i => i.backImage).filter(i => !!i)])

    const [isContinue, setContinue] = useState(false)
    const [isRePlay, setRePlay] = useState(false)

    const [index, setIndex] = useState(
        histories == cards.length ? 0 : histories >= cardFull.length ? numberFormat : histories % cards?.length
        // numberFormat
    )
    ...
    useEffect(() => {
        ...
        const ques = question
        ques.type =
            cards[index]?.types[Math.floor(Math.random() * (cards[index]?.types.length || 0))] ||
        ...
        let randAnswers = shuffle(ques.answerType === 'image' ? cardImgs.current : cardTexts.current)
        if (ques.type == 'abcd') {
            randAnswers = shuffle(backAnswersText.current)
            ques.answerTrue = ques.questionContent
        } else {
            randAnswers = shuffle(ques.answerType === 'image' ? cardImgs.current : cardTexts.current)
        }
        const _answers = randAnswers.filter(i => i !== ques.answerTrue).slice(0, 3)
    ...
    useEffect(() => {
        if (isCorrect != null) {
            ...
            const dataset = {
                ...
                flashcards: [
                    {
                        id: isContinue ? cardFull[index].id : isRePlay ? cardFull[index].id : cardFull[index].id,
                        result: isCorrect,
                    },
                ],
            }
    ...
    return (
        <div className='flex flex-1 flex-col'>
            ...
            {question.type === 'blank' && <Blank isCorrect={isCorrect} onAnswer={ans => handleAnswer(ans)()} />}
        </div>
    )
}
