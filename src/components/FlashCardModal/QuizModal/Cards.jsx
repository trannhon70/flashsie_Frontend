...
const Cards = ({ data = [], flashsetId, onClose }) => {
    ...
    const card = useRef()
    const cardImgs = useRef([...data.map(i => i.frontImage), ...data.map(i => i.backImage)].filter(i => !!i))
    const cardTexts = useRef([...data.map(i => i.frontText), ...data.map(i => i.backText)].filter(i => !!i))
    ...
    return (
        <div className='flex flex-col'>
            ...
            <div className='flex flex-row justify-center gap-4 p-4 pb-4'>
                {cards.length - index >= 1 && (
                    <Answers
                        answer={question.answerTrue.split('').map((i, idx) => ({ key: idx, value: i.toUpperCase() }))}
