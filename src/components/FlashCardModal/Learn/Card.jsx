...
import { speak, speakAudio, useWindowSize } from '@/utils/helpers'

const Card = forwardRef(({ data, disableFlip, onFlip, onNext, onPrev, onSpeak, onSpeaked, autoSpeak, isSwap }, ref) => {
    ...
    return (
        ...
        >
            <ReactCardFlip containerClassName='h-full w-full' isFlipped={isFlipped} flipDirection='horizontal'>
                <CardItem
                    isSwap={isSwap}
...
const CardItem = ({ image, text, color, isCorrect, onSpeak, onSpeaked, autoSpeak, isColorCard, isSwap }) => {
    useEffect(() => {
        if (autoSpeak) speakAudio(text)
    }, [text])
    ...
    if (!image && text) {
        return (
            <div
                ...
                style={{
                    backgroundColor: isCorrect == true ? '#01ac47' : isCorrect == false ? '#bf0118' : color,
                }}
            >
                <div
    ...
    return (
        ...
        >
            ...
            <Button className='absolute left-2 top-2' color='primary' isIconOnly variant='light' onPress={handleSpeak}>
                <FaVolumeLow size={30} />
            </Button>
        </div>
    )
